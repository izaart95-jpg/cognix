...
pub const G4F_API_URL: &str = "http://localhost:8080";
...
pub struct ChatTemplateCaps {
    /// g4f's `/props` reports both of these keys, so we read them into
...
impl LoadProgress {
    /// A human label for the stage currently loading, matching the g4f
...
pub async fn get_models(
    ...
    let response: ListModelsResponse =
        serde_json::from_str(&body).context("Unable to parse g4f models response")?;
    Ok(response.data)
}
...
pub async fn get_props(
    ...
    let props: Props =
        serde_json::from_str(&body).context("Unable to parse g4f props response")?;
    Ok(props)
}
...
pub async fn stream_model_events(
    ...
    if !response.status().is_success() {
        ...
        response.body_mut().read_to_string(&mut body).await?;
        anyhow::bail!(
            "Failed to open g4f model event stream: {} {}",
            response.status(),
            body,
        );
    }
