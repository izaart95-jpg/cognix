//! Paths to locations used by Cognix.
...
/// On macOS, this is `~/Library/Application Support/Cognix`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/cognix`.
/// On Windows, this is `%LOCALAPPDATA%\Cognix`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/cognix`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/cognix`.
/// On Windows, this is `%APPDATA%\Cognix`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the cognix_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".cognix_server").unwrap());
    *CACHED
}

// Remove this once 223 goes stable
/// Returns the relative path to the cognix_wsl_server directory on the wsl host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".cognix_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by Cognix.
...
/// Returns the path to the data directory used by Cognix.
...
/// Returns the path to the temp directory used by Cognix.
...
/// Returns the path to the Cognix server directory on this SSH host.
...
/// Returns the path to the `Cognix.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the `Cognix.log.old` file.
...
/// user opens, and is loaded into the native agent's system prompt.
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Cognix repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Cognix.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Cognix.
...
/// Returns the relative path to a `.cognix` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".cognix/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".cognix/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".cognix/debug.json").unwrap());
    *CACHED
}
