...
const LLAMA_CPP_CLONE_DOWNLOAD_URL: &str = "https://github.com/izaart95-jpg/GLM-Free-API";
const LLAMA_CPP_CLONE_MODELS_URL: &str = "https://openrouter.ai/z-ai";
...
pub struct LlamaCppCloneSettings {
    pub api_url: String,
...
pub struct LlamaCppCloneLanguageModelProvider {
...
pub struct State {
    ...
    http_client: Arc<dyn HttpClient>,
    fetched_models: Vec<llama_cpp_clone::Model>,
...
impl State {
    ...
    fn set_api_key(&mut self, api_key: Option<String>, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        let api_url = LlamaCppCloneLanguageModelProvider::api_url(cx);
    ...
    fn authenticate(&mut self, cx: &mut Context<Self>) -> Task<Result<(), AuthenticateError>> {
        ...
        cx.spawn(async move |this, cx| {
            match load_key_task.await {
                ...
                Err(error) => {
                    log::warn!("failed to load llama.cpp_clone API key: {error}");
                }
            }
    ...
    fn fetch_models(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        let settings = LlamaCppCloneLanguageModelProvider::settings(cx);
        let api_url = LlamaCppCloneLanguageModelProvider::api_url(cx);
        ...
        cx.spawn(async move |this, cx| {
            ...
            let models: Vec<llama_cpp_clone::Model> = if is_router {
            ...
            this.update(cx, |this, cx| {
                ...
                let effective = compute_effective_models(
                    &this.fetched_models,
                    LlamaCppCloneLanguageModelProvider::settings(cx),
                );
    ...
    fn start_model_event_stream(&mut self, cx: &mut Context<Self>) {
        ...
        let api_url = LlamaCppCloneLanguageModelProvider::api_url(cx);
        let api_key = self.api_key_state.key(&api_url).or_else(|| Some("Waguri".to_string().into()));
        let extra_headers = LlamaCppCloneLanguageModelProvider::settings(cx)
            .custom_headers
            .clone();
        ...
        self.model_event_task = Some(cx.spawn(async move |this, cx| {
            loop {
                ...
                .await
                {
                    Ok(mut events) => {
                        while let Some(event) = events.next().await {
                            ...
                            if let Some(exit_code) = event.load_failure() {
                                log::error!(
                                    "llama.cpp_clone model {} failed to load (exit code {exit_code})",
                                    event.model
                                );
                            }
                    ...
                    Err(error) => {
                        log::warn!("llama.cpp_clone model event stream unavailable: {error:#}");
                    }
                }
...
impl LiveCapabilities {
    fn of(model: &llama_cpp_clone::Model) -> Self {
        Self {
...
fn compute_effective_models(
    fetched_models: &[llama_cpp_clone::Model],
    settings: &LlamaCppCloneSettings,
) -> HashMap<String, llama_cpp_clone::Model> {
    let mut models: HashMap<String, llama_cpp_clone::Model> = HashMap::default();
...
fn sync_capability_cells(cells: &CapabilityCells, effective: &HashMap<String, llama_cpp_clone::Model>) {
...
fn model_from_entry(entry: &ModelEntry, props: Option<&Props>) -> llama_cpp_clone::Model {
...
impl LlamaCppCloneLanguageModelProvider {
    pub fn new(
        http_client: Arc<dyn HttpClient>,
        credentials_provider: Arc<dyn CredentialsProvider>,
        cx: &mut App,
        ...
        let this = Self {
            ...
            state: cx.new(|cx| {
                cx.observe_global::<SettingsStore>({
                    let mut last_settings = LlamaCppCloneLanguageModelProvider::settings(cx).clone();
                    move |this: &mut State, cx| {
                        let current_settings = LlamaCppCloneLanguageModelProvider::settings(cx);
    ...
    fn settings(cx: &App) -> &LlamaCppCloneSettings {
        &AllLanguageModelSettings::get_global(cx).llama_cpp_clone
    }
    ...
    fn has_custom_url(cx: &App) -> bool {
        let api_url = &Self::settings(cx).api_url;
        !api_url.is_empty() && api_url != LLAMA_CPP_API_URL
    }
}

impl LanguageModelProviderState for LlamaCppCloneLanguageModelProvider {
...
impl LanguageModelProvider for LlamaCppCloneLanguageModelProvider {
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        let mut models = effective
            .into_values()
            .map(|model| {
                Arc::new(LlamaCppCloneLanguageModel {
    ...
    fn settings_view(&self, _cx: &mut App) -> Option<ProviderSettingsView> {
        ...
        Some(ProviderSettingsView::SubPage(
            ...
            .description(InlineDescription::Text(
                "Run GLM models on your machine with GLMFreeAPI.".into(),
            )),
        ))
    }
}

pub struct LlamaCppCloneLanguageModel {
    id: LanguageModelId,
    name: String,
    display_name: String,
    capability_cells: CapabilityCells,
...
impl LlamaCppCloneLanguageModel {
    ...
    fn to_llama_cpp_clone_request(
        &self,
        request: LanguageModelRequest,
    ) -> Result<llama_cpp_clone::ChatCompletionRequest> {
        build_llama_cpp_clone_request(
            &self.name,
    ...
    fn stream_completion(
        &self,
        request: llama_cpp_clone::ChatCompletionRequest,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<futures::stream::BoxStream<'static, Result<llama_cpp_clone::ResponseStreamEvent>>>,
    > {
...
fn build_llama_cpp_clone_request(
    model_name: &str,
    supports_images: bool,
    ...
    if request.contains_custom_tool_input() {
        anyhow::bail!("llama.cpp_clone does not support custom tools");
    }
        ...
        for content in message.content {
            match content {
                MessageContent::Text(text) => add_message_content_part(
                    llama_cpp_clone::MessagePart::Text { text },
                    message.role,
                    &mut messages,
                ...
                MessageContent::Image(image) => {
                    if supports_images {
                        add_message_content_part(
                            llama_cpp_clone::MessagePart::Image {
                                image_url: llama_cpp_clone::ImageUrl {
                ...
                MessageContent::ToolUse(tool_use) => {
                    let input = tool_use.input.as_json().ok_or_else(|| {
                        anyhow::anyhow!("llama.cpp_clone does not support custom tool calls")
                    })?;
                    let tool_call = llama_cpp_clone::ToolCall {
                        id: tool_use.id.to_string(),
                        content: llama_cpp_clone::ToolCallContent::Function {
                            function: llama_cpp_clone::FunctionContent {
                    ...
                    if let Some(llama_cpp_clone::ChatMessage::Assistant {
                        tool_calls,
                    ...
                    }) = messages.last_mut()
                    {
                        ...
                        messages.push(llama_cpp_clone::ChatMessage::Assistant {
                            content: None,
                ...
                MessageContent::ToolResult(tool_result) => {
                    let content: Vec<llama_cpp_clone::MessagePart> = tool_result
                        .content
                        .iter()
                        .filter_map(|part| match part {
                            LanguageModelToolResultContent::Text(text) => {
                                Some(llama_cpp_clone::MessagePart::Text {
                                    text: text.to_string(),
                                })
                            }
                            LanguageModelToolResultContent::Image(image) => {
                                if supports_images {
                                    Some(llama_cpp_clone::MessagePart::Image {
                                        image_url: llama_cpp_clone::ImageUrl {
                                            url: image.to_base64_url(),
                                            detail: None,
                                        },
                    ...
                    messages.push(llama_cpp_clone::ChatMessage::Tool {
    ...
    let tools: Vec<llama_cpp_clone::ToolDefinition> = if supports_tools {
        request
            .tools
            .into_iter()
            .map(|tool| {
                let input_schema = match tool.input {
                    ...
                    language_model::LanguageModelRequestToolInput::Custom { .. } => {
                        return Err(anyhow::anyhow!("llama.cpp_clone does not support custom tools"));
                    }
                };
                Ok(llama_cpp_clone::ToolDefinition::Function {
                    function: llama_cpp_clone::FunctionDefinition {
                        name: tool.name,
    ...
    let tool_choice = if tools.is_empty() {
        ...
        request.tool_choice.map(|choice| match choice {
            LanguageModelToolChoice::Auto => llama_cpp_clone::ToolChoice::Auto,
            LanguageModelToolChoice::Any => llama_cpp_clone::ToolChoice::Required,
            LanguageModelToolChoice::None => llama_cpp_clone::ToolChoice::None,
        })
    };

    Ok(llama_cpp_clone::ChatCompletionRequest {
        ...
        tools,
        tool_choice,
        stream_options: Some(llama_cpp_clone::StreamOptions {
            include_usage: true,
        }),
        reasoning_effort,
    })
...
impl LanguageModel for LlamaCppCloneLanguageModel {
    ...
    fn stream_completion(
        ...
        >,
    > {
        let request = match self.to_llama_cpp_clone_request(request) {
            Ok(request) => request,
            Err(error) => return async move { Err(error.into()) }.boxed(),
        };
        let completions = self.stream_completion(request, cx);
        async move {
            let mapper = LlamaCppCloneEventMapper::new();
...
struct LlamaCppCloneEventMapper {
    tool_calls_by_index: HashMap<usize, RawToolCall>,
}

impl LlamaCppCloneEventMapper {
    fn new() -> Self {
        Self {
    ...
    pub fn map_stream(
        mut self,
        events: Pin<Box<dyn Send + Stream<Item = Result<llama_cpp_clone::ResponseStreamEvent>>>>,
    ...
    pub fn map_event(
        &mut self,
        event: llama_cpp_clone::ResponseStreamEvent,
        ...
        if let Some(choice) = event.choices.into_iter().next() {
            ...
            if let Some(finish_reason) = choice.finish_reason.as_deref() {
                match finish_reason {
                    ...
                    unexpected => {
                        log::warn!("Unexpected llama.cpp_clone finish_reason: {unexpected:?}");
                        events.push(Ok(LanguageModelCompletionEvent::Stop(StopReason::EndTurn)));
                    }
                }
            }
...
fn add_message_content_part(
    new_part: llama_cpp_clone::MessagePart,
    role: Role,
    messages: &mut Vec<llama_cpp_clone::ChatMessage>,
    reasoning_content: Option<String>,
) {
    match (role, messages.last_mut()) {
        (Role::User, Some(llama_cpp_clone::ChatMessage::User { content }))
        | (Role::System, Some(llama_cpp_clone::ChatMessage::System { content })) => {
            content.push_part(new_part);
        }
        (
            Role::Assistant,
            Some(llama_cpp_clone::ChatMessage::Assistant {
        ...
        _ => {
            messages.push(match role {
                Role::User => llama_cpp_clone::ChatMessage::User {
                    content: llama_cpp_clone::MessageContent::from(vec![new_part]),
                },
                Role::Assistant => llama_cpp_clone::ChatMessage::Assistant {
                    content: Some(llama_cpp_clone::MessageContent::from(vec![new_part])),
                    reasoning_content,
                    tool_calls: Vec::new(),
                },
                Role::System => llama_cpp_clone::ChatMessage::System {
                    content: llama_cpp_clone::MessageContent::from(vec![new_part]),
                },
            });
        }
...
fn merge_settings_into_models(
    models: &mut HashMap<String, llama_cpp_clone::Model>,
...
impl ConfigurationView {
    pub fn new(state: Entity<State>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let api_url_editor = cx.new(|cx| {
            ...
            input.set_text(&LlamaCppCloneLanguageModelProvider::api_url(cx), window, cx);
            input
        });

        let context_window_editor = cx.new(|cx| {
            let input = InputField::new(window, cx, "8192").label("Context Window");
            if let Some(context_window) = LlamaCppCloneLanguageModelProvider::settings(cx).context_window
            {
    ...
    fn retry_connection(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let has_api_url = LlamaCppCloneLanguageModelProvider::has_custom_url(cx);
    ...
    fn save_api_url(&self, cx: &mut Context<Self>) {
        ...
        let current_url = LlamaCppCloneLanguageModelProvider::api_url(cx);
    ...
    fn reset_api_url(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        update_settings_file(fs, cx, |settings, _cx| {
            if let Some(settings) = settings
                .language_models
                .as_mut()
                .and_then(|models| models.llama_cpp_clone.as_mut())
            {
    ...
    fn save_context_window(&mut self, cx: &mut Context<Self>) {
        let context_window_str = self
            ...
            .trim()
            .to_string();
        let current_context_window = LlamaCppCloneLanguageModelProvider::settings(cx).context_window;
    ...
    fn render_instructions(cx: &App) -> Div {
        v_flex()
            ...
            .child(
                Label::new(
                    "Run GLM models locally with GLMFreeAPI, or connect to a \
                remote GLMFreeAPI server.",
                )
                .color(Color::Muted),
            )
            .child(Label::new("To use a local GLMFreeAPI server:").color(Color::Muted))
            .child(
                List::new()
                    .child(
                        ListBulletItem::new("")
                            .child(Label::new("Setup GLMFreeAPI from").color(Color::Muted))
                            .child(ButtonLink::new("GLMFreeAPI", LLAMA_CPP_CLONE_DOWNLOAD_URL)),
                    )
                    .child(
                        ListBulletItem::new("")
                            .child(
                                Label::new("Start the server in agent mode:").color(Color::Muted),
                            )
                            .child(Label::new("go run main.go --agent-mode").inline_code(cx)),
                    )
                    .child(
                        ListBulletItem::new(
                            "Click 'Connect' below to start using GLMFreeAPI in Cognix",
                        )
            ...
            .child(
                Label::new(
                    "Alternatively, you can connect to the default remote GLMFreeAPI server by leaving the \
...
impl Render for ConfigurationView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            .gap_2()
            .child(Headline::new("GLMFreeAPI").size(HeadlineSize::Small))
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                                    ...
                                    this.child(
                                        Button::new("llama-cpp-clone-webui", "Check API health")
                                            ...
                                            .on_click(move |_, _, cx| {
                                                let url =
                                                    LlamaCppCloneLanguageModelProvider::api_url(cx);
                                                cx.open_url(&url);
                                            })
                                            .into_any_element(),
                                    )
                                    .child(
                                        Button::new("llama-cpp-clone-site", "GLMFreeAPI")
                                            .style(ButtonStyle::OutlinedGhost)
                                            .size(ButtonSize::Medium)
                                            .end_icon(
                                                Icon::new(IconName::ArrowUpRight)
                                            ...
                                            .on_click(move |_, _, cx| {
                                                cx.open_url(LLAMA_CPP_CLONE_DOWNLOAD_URL)
                                            })
                                            .into_any_element(),
                                    )
                                } else {
                                    this.child(
                                        Button::new("download_llama_cpp_clone_button", "Get GLMFreeAPI")
                                            .style(ButtonStyle::OutlinedGhost)
                                            .size(ButtonSize::Medium)
                                            .end_icon(
                                                Icon::new(IconName::ArrowUpRight)
                            ...
                            .child(
                                Button::new("view-models", "Browse GLM Models")
                                    .style(ButtonStyle::OutlinedGhost)
                                    .size(ButtonSize::Medium)
                                    .end_icon(
                                        Icon::new(IconName::ArrowUpRight)
                                    ...
                                    .on_click(move |_, _, cx| cx.open_url(LLAMA_CPP_CLONE_MODELS_URL)),
                            ),
                    )
                    .map(|this| {
                        if is_authenticated {
